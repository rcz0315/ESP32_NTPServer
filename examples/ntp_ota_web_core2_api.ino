...
#include "html_mini.h"           // Include the HTML header
...
void setup() {
  ...
  server.on("/api/data", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    jsonDoc["GPStime"] = String(timeBuf);
  ...
  server.on("/api/restart", HTTP_POST, [](AsyncWebServerRequest *request) {
    AsyncWebServerResponse *response = request->beginResponse(200, "application/json", R"({"status":"restarting"})");
    addCORS(response);
    request->send(response);
  ...
  server.on(
    ...
    [](AsyncWebServerRequest *request, const String &filename, size_t index, uint8_t *data, size_t len, bool final) {
      ...
      if (final) {
        if (Update.end(true)) {
          AsyncWebServerResponse *response = request->beginResponse(200, "application/json", "{\"status\":\"ok\",\"message\":\"update complete\"}");
          addCORS(response);
          request->send(response);
          restartRequested = true;
          restartTime = millis() + 1000;
        } else {
          AsyncWebServerResponse *response = request->beginResponse(500, "application/json", "{\"status\":\"fail\",\"message\":\"end error\"}");
          addCORS(response);
          request->send(response);
        }
      }
