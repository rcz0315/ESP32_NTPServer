...
NTPServer::NTPServer(const char *referenceId, const char stratum)
  : NTPServer() {
    ...
    if (_u_packetBuffer.header.vn >= L_NTP_MIN_VER || _u_packetBuffer.header.vn <= L_NTP_MAX_VER) {
      ...
      } else if (_u_packetBuffer.header.mode == L_NTP_MODE_CONTROL) /* Control Mode Request */
      {
        ...
        if (_recv(sizeof(S_NTP_CONTROL_PACKET) - sizeof(S_NTP_HEADER))) {
          ...
          if (_u_packetBuffer.controlPacket.count >= 0 && _u_packetBuffer.controlPacket.count <= L_NTP_MAX_RX_BUFF - sizeof(S_NTP_CONTROL_PACKET) && _u_packetBuffer.controlPacket.response == 0 && _u_packetBuffer.controlPacket.error == 0 && _u_packetBuffer.controlPacket.more == 0) {
